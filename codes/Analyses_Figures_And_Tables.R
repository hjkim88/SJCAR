...
#               37. Try Leave One Patient Out Cross Validation on the classifier with Precursor signature
#
#   Instruction
...
manuscript_prep <- function(Seurat_RObj_path="./data/NEW_SJCAR_SEURAT_OBJ/SJCAR19_Oct2020_Seurat_Obj_Total2.RDS",
  ...
  p <- ggplot(data=plot_df, aes_string(x="Time_Point", y="Numbers", fill="State", label="Pcnt")) +
    ...
    ggtitle("") +
    xlab("Time") + ylab("Cell #") +
    geom_text(size = 6, position = position_stack(vjust = 0.5), color = "cornsilk2", fontface = 2) +
    coord_flip() +
  ...
  ### now the x-axis is the pcnt2
  p <- ggplot(data=plot_df, aes_string(x="Time_Point", y="Pcnt2", fill="State", label="Pcnt")) +
    geom_bar(position = "stack", stat = "identity") +
    ggtitle("") +
    ...
    theme(plot.title = element_text(hjust = 0.5, vjust = 0.5, size = 30, color = "black", face = "bold"),
          ...
          legend.key.size = unit(2, 'cm'),
          axis.ticks = element_blank())
  ggsave(file = paste0(outputDir2, "CARpos_Trajectory_Inference_Monocle2_Bar_Subsisters_Added(2)_Fig3C_Pcnt.pdf"), plot = p,
         width = 18, height = 10, dpi = 350)
  
  ### in the inverse way - state -> time
  
  ### calculate percentages
  st_sum <- rep(0, length(levels(monocle_cds2$State)))
  names(st_sum) <- levels(monocle_cds2$State)
  for(i in 1:length(levels(monocle_cds2$State))) {
    st_sum[i] <- sum(plot_df[which(plot_df$State == levels(monocle_cds2$State)[i]),"Numbers"])
    plot_df$Pcnt[which(plot_df$State == levels(monocle_cds2$State)[i])] <- round(plot_df$Numbers[which(plot_df$State == levels(monocle_cds2$State)[i])] * 100 / st_sum[i], 1)
  }
  ...
  ### color scale
  sjcar19_colors <- rev(c("#640B11", "#AA4C26", "#D39F3A", "#C09969", "#287B66", "#487A8F", "#3B3B53"))
  names(sjcar19_colors) <- levels(plot_df$Time_Point)
  show_col(sjcar19_colors)
  
  ### preserve the original pcnt
  plot_df$Pcnt2 <- as.numeric(plot_df$Pcnt)
  ...
  ### Numbers < 50 -> ""
  plot_df$Pcnt[which(as.numeric(plot_df$Numbers) < 50)] <- ""
  plot_df$Pcnt <- as.character(plot_df$Pcnt)
  ...
  p <- ggplot(data=plot_df, aes_string(x="State", y="Numbers", fill="Time_Point", label="Pcnt")) +
    ...
    geom_text(size = 6, position = position_stack(vjust = 0.5), color = "cornsilk2", fontface = 2) +
    coord_flip() +
    scale_fill_manual(values = sjcar19_colors, name = "Time") +
    guides(color = guide_legend(override.aes = list(size = 20))) +
    theme_classic(base_size = 35) +
    theme(plot.title = element_text(hjust = 0.5, vjust = 0.5, size = 30, color = "black", face = "bold"),
          axis.text = element_text(size = 20, color = "black", face = "bold"),
          ...
          legend.key.size = unit(2, 'cm'),
          axis.ticks = element_blank())
  ggsave(file = paste0(outputDir2, "CARpos_Trajectory_Inference_Monocle2_Bar_Subsisters_Added(2)_Fig3C_Inverse.pdf"), plot = p,
         width = 18, height = 10, dpi = 350)
  
  ### different label filtering
  plot_df$Pcnt <- plot_df$Pcnt2
  ...
  ### now the x-axis is the pcnt2
  p <- ggplot(data=plot_df, aes_string(x="State", y="Pcnt2", fill="Time_Point", label="Pcnt")) +
    geom_bar(position = "stack", stat = "identity") +
    ggtitle("") +
    ...
    theme(plot.title = element_text(hjust = 0.5, vjust = 0.5, size = 30, color = "black", face = "bold"),
          ...
          legend.key.size = unit(2, 'cm'),
          axis.ticks = element_blank())
  ggsave(file = paste0(outputDir2, "CARpos_Trajectory_Inference_Monocle2_Bar_Subsisters_Added(2)_Fig3C_Inverse_Pcnt.pdf"), plot = p,
         width = 18, height = 10, dpi = 350)
  ...
  sjcar19_color_scale <- c("white", "#C09969", "#640B11", "#287B66", "#D39F3A", "#487A8F", "#AA4C26", "#3B3B53")
  names(sjcar19_color_scale) <- total_clusters
  show_col(sjcar19_color_scale)
  
  ### node and edge width + colors
  E(g)$width <- E(g)$weight + 5
  E(g)$edgeColor <- c(rep("#25245D", length(which(adj_mat["GMP",] > 0))), rep("#CAA638", length(E(g))-length(which(adj_mat["GMP",] > 0))))
  ...
  sjcar19_colors <- rev(c("#640B11", "#AA4C26", "#D94C21", "#D39F3A", "#C09969", "#287B66", "#286278", "#487A8F", "#3B3B53", "#25245D"))
  ...
  ggsave(paste0(outputDir2, "UMAP_CARpos_Density.pdf"), plot = p, width = 15, height = 10, dpi = 350)
  ...
  ### the column must only contain Wk1-Wk8
  JCC_Seurat_Obj$Seurat_Clusters_Subsisters3 <- JCC_Seurat_Obj$Seurat_Clusters_Subsisters2
  JCC_Seurat_Obj$Seurat_Clusters_Subsisters3[which(JCC_Seurat_Obj$Seurat_Clusters_Subsisters3 %in% c("Non-Subsisters", "GMP"))] <- "Others"
  
  ### color scale
  sjcar19_colors <- c("#640B11", "#D39F3A", "#C09969", "#287B66", "#487A8F", "lightgray")
  names(sjcar19_colors) <- c("Wk1", "Wk2", "Wk3", "Wk4", "Wk8", "Others")
  show_col(sjcar19_colors)
  ...
  ggsave(paste0(outputDir2, "UMAP_CARpos_Subsisters_By_Time_Arrow_PI_ONLY_Fig4B.pdf"), plot = p, width = 13, height = 10, dpi = 350)
  ...
  names(sjcar19_colors) <- c("GMP", "Wk1", "Wk2", "Wk3", "Wk4", "Wk8", "Others")
  show_col(sjcar19_colors)
  
  ### add arrows to the previous UMAP
  JCC_Seurat_Obj$Seurat_Clusters_Subsisters2[which(JCC_Seurat_Obj$Seurat_Clusters_Subsisters2 == "Non-Subsisters")] <- "Others"
  p <- DimPlot(object = JCC_Seurat_Obj, reduction = "umap", raster = TRUE,
               group.by = "Seurat_Clusters_Subsisters2",
               pt.size = 5,
               cols = sjcar19_colors,
  ...
  ggsave(paste0(outputDir2, "UMAP_CARpos_Subsisters_By_Time_Arrow_PI_ONLY_With_GMP_Fig4C.pdf"), plot = p, width = 13, height = 10, dpi = 350)
  ...
  ### there are some CD8-CD4 mixed lineages
  ### we only use all at least one CD4 cell in each of GMP & PI
  gmp_cd4_subsisters_clones <- intersect(gmp_cd4_subsisters_clones, pi_cd4_subsister_clones)
  pi_cd4_subsister_clones <- intersect(pi_cd4_subsister_clones, gmp_cd4_subsisters_clones)
  ...
  ggsave(paste0(outputDir2, "UMAP_CARpos_Subsisters_By_Time_Arrow_ALL_CD4s.pdf"), plot = p, width = 15, height = 10, dpi = 350)
  ...
  ### check whether there are PI-PI CD4 lineages exist
  ### found there is no PI-PI interactions, only GMP-PI in CD4 lineages
  for(i in 1:length(gmp_cd4_subsisters_clones)) {
    target_indicies <- which(JCC_Seurat_Obj$clonotype_id_by_patient_one_alpha_beta == gmp_cd4_subsisters_clones[i])
    existing_time <- unique(JCC_Seurat_Obj$time2[target_indicies])
    if(length(existing_time) > 1) {
      writeLines(paste(existing_time, collapse = ","))
    }
  }
  ...
  ### without GMP cells
  monocle_cds3 <- monocle_cds2
  monocle_cds3@phenoData@data <- monocle_cds3@phenoData@data[which(monocle_cds3@phenoData@data$GMP != "GMP"),]
  monocle_cds3@reducedDimS <- monocle_cds3@reducedDimS[,rownames(monocle_cds3@phenoData@data)]
  monocle_cds3@assayData$exprs <- monocle_cds3@assayData$exprs[,rownames(monocle_cds3@phenoData@data)]
  ...
  ### draw a gene expression monocle plot
  p <- plot_cell_trajectory(monocle_cds3, markers = interesting_genes, use_color_gradient = TRUE,
                            cell_size = 3, cell_link_size = 1, show_branch_points = FALSE) +
    labs(color="Normalized\nExpression") +
    scale_color_gradientn(colours = add.alpha(c("#487A8F", "#D39F3A", "#640B11"), alpha = 0.7)) +
    theme_classic(base_size = 40) +
    theme(legend.key.size = unit(3, 'cm'),
          legend.position = "right",
          legend.title = element_text(angle = 0, size = 30, vjust = 0.5, hjust = 0.5, color = "black", face = "bold"),
          ...
          axis.title = element_text(angle = 0, size = 30, vjust = 0.5, hjust = 0.5, color = "black", face = "bold"),
          axis.text = element_text(angle = 0, size = 25, vjust = 0.5, hjust = 0.5, color = "black", face = "bold"))
  ggsave(file = paste0(outputDir2, "GEXP_Monocle2_Subsisters_Added_Fig3C_No_GMP.pdf"),
         plot = p,
         width = 30, height = 10, dpi = 350)
  
  ### pseudotime map with cluster info
  cluster_color_scale <- c("#16101E", "#D0B78F", "#8C8781", "#C7904F", "#133D31", "#82A5B8", "#3B3B53", "#4C8493", "#C31517",
                           "#D94C21", "#3E89A8", "#AA4C26",  "#CAA638", "#640B11", "#629488", "#BD7897", "#3C2D16", "#25245D",
                           "#E64E46", "#73BCAA", "#7047C1", "#286278")
  cluster_color_scale <- add.alpha(cluster_color_scale, alpha = 0.8)
  names(cluster_color_scale) <- levels(JCC_Seurat_Obj$AllSeuratClusters)
  p <- plot_cell_trajectory(monocle_cds3, color_by = "AllSeuratClusters", cell_size = 3, cell_link_size = 3, show_branch_points = FALSE) +
    labs(color="") +
    scale_color_manual(labels = levels(JCC_Seurat_Obj$AllSeuratClusters), values = cluster_color_scale, name = "Clusters") +
    guides(color = guide_legend(override.aes = list(size = 10))) +
    theme_classic(base_size = 36) +
    theme(legend.position = "top",
          plot.title = element_text(hjust = 0, vjust = 0.5, size = 30, color = "black", face = "bold"),
          ...
          axis.text = element_text(angle = 0, size = 25, vjust = 0.5, hjust = 0.5, color = "black", face = "bold")) +
    facet_wrap(~AllSeuratClusters, ncol = 7)
  ggsave(file = paste0(outputDir2, "CARpos_Trajectory_Subsisters_Added_Clusters_No_GMP2.pdf"),
         plot = p,
         width = 20, height = 15, dpi = 350)
  
  ### pseudotime map with real time
  time_colors <- rev(c("#640B11", "#AA4C26", "#D39F3A", "#C09969", "#287B66", "#487A8F"))
  time_colors <- add.alpha(time_colors, alpha = 0.8)
  names(time_colors) <- unique(monocle_cds3$time2)
  p <- plot_cell_trajectory(monocle_cds3, color_by = "time2", cell_size = 3, cell_link_size = 3, show_branch_points = FALSE) +
    labs(color="") +
    scale_color_manual(labels = names(time_colors), values = time_colors, name = "Time") +
    guides(color = guide_legend(override.aes = list(size = 10))) +
    theme_classic(base_size = 36) +
    theme(legend.position = "top",
          plot.title = element_text(hjust = 0, vjust = 0.5, size = 30, color = "black", face = "bold"),
  ...
  ggsave(file = paste0(outputDir2, "CARpos_Trajectory_Subsisters_Added_Time_No_GMP2.pdf"),
         plot = p,
         width = 16, height = 12, dpi = 350)
  
  ### pseudotime map with state
  state_colors <- c("#640B11", "#AA4C26", "#D39F3A", "#C09969", "#287B66", "#487A8F", "#3B3B53")
  state_colors <- add.alpha(state_colors, alpha = 0.8)
  names(state_colors) <- levels(monocle_cds3$State)
  p <- plot_cell_trajectory(monocle_cds3, color_by = "State", cell_size = 3, cell_link_size = 3, show_branch_points = FALSE) +
    labs(color="") +
    scale_color_manual(labels = names(state_colors), values = state_colors, name = "State") +
    guides(color = guide_legend(override.aes = list(size = 10))) +
    theme_classic(base_size = 36) +
    theme(legend.position = "top",
          plot.title = element_text(hjust = 0, vjust = 0.5, size = 30, color = "black", face = "bold"),
          ...
          axis.title = element_text(angle = 0, size = 30, vjust = 0.5, hjust = 0.5, color = "black", face = "bold"),
          axis.text = element_text(angle = 0, size = 25, vjust = 0.5, hjust = 0.5, color = "black", face = "bold"))
  ggsave(file = paste0(outputDir2, "CARpos_Trajectory_Subsisters_Added_State_No_GMP.pdf"),
         plot = p,
         width = 13, height = 10, dpi = 350)
  ...
  p <- DotPlot(downsampled_seurat_obj,
    ...
    theme(plot.title = element_text(hjust = 0.5, color = "black", face = "bold"),
          ...
          legend.text = element_text(size = 25, color = "black", face = "bold"),
          legend.key.size = unit(0.7, 'cm'))
  ggsave(file = paste0(outputDir2, "Dotplot_GEXP_Monocle2_Subsisters_Added_Fig3C.pdf"),
         plot = p, width = 18, height = 10, dpi = 350)
  
  #
  ### Dotplot with other genes
  #
  ...
  p <- DotPlot(downsampled_seurat_obj,
    ...
    theme(plot.title = element_text(hjust = 0.5, color = "black", face = "bold"),
          ...
          legend.text = element_text(size = 25, color = "black", face = "bold"),
          legend.key.size = unit(0.7, 'cm'))
  ggsave(file = paste0(outputDir2, "Dotplot_GEXP_Monocle2_Subsisters_Added_Tay_Requested_Fig3D.pdf"),
         plot = p, width = 18, height = 10, dpi = 350)
  ...
  ### addmodulescore creates different column than designated
  ### we are interested in GMP cells only
  JCC_Seurat_Obj$GMP_Subsisters_End_Up_In_Cluster38_2_CD8_Module_Score <- JCC_Seurat_Obj$GMP_Subsisters_End_Up_In_Cluster38_2_CD8_Module_Score1
  JCC_Seurat_Obj$GMP_Subsisters_End_Up_In_Cluster38_2_CD8_Module_Score[which(JCC_Seurat_Obj$GMP == "PI")] <- NA
  JCC_Seurat_Obj$GMP_Subsisters_End_Up_In_Cluster38_2_CD8_Module_Score2 <- JCC_Seurat_Obj$GMP_Subsisters_End_Up_In_Cluster38_2_CD8_Module_Score21
  JCC_Seurat_Obj$GMP_Subsisters_End_Up_In_Cluster38_2_CD8_Module_Score2[which(JCC_Seurat_Obj$GMP == "PI")] <- NA
  ...
  JCC_Seurat_Obj$GMP_Subsisters_End_Up_In_Cluster38_2_CD8_Module_Score1_Cells <- "NO_GMP"
  JCC_Seurat_Obj$GMP_Subsisters_End_Up_In_Cluster38_2_CD8_Module_Score1_Cells[intersect(which(JCC_Seurat_Obj$GMP_Subsisters_End_Up_In_Cluster38_2_CD8_Module_Score1 > positive_module_score_threshold),
                                                                                        which(JCC_Seurat_Obj$time2 == "GMP"))] <- "YES"
  JCC_Seurat_Obj$GMP_Subsisters_End_Up_In_Cluster38_2_CD8_Module_Score1_Cells[intersect(which(JCC_Seurat_Obj$GMP_Subsisters_End_Up_In_Cluster38_2_CD8_Module_Score1 <= positive_module_score_threshold),
                                                                                        which(JCC_Seurat_Obj$time2 == "GMP"))] <- "NO"
  JCC_Seurat_Obj$GMP_CARpos_Persister_Module_Score1_Cells <- "NO_GMP"
  JCC_Seurat_Obj$GMP_CARpos_Persister_Module_Score1_Cells[intersect(which(JCC_Seurat_Obj$GMP_CARpos_Persister_Module_Score1 > positive_module_score_threshold),
                                                                    which(JCC_Seurat_Obj$time2 == "GMP"))] <- "YES"
  JCC_Seurat_Obj$GMP_CARpos_Persister_Module_Score1_Cells[intersect(which(JCC_Seurat_Obj$GMP_CARpos_Persister_Module_Score1 <= positive_module_score_threshold),
                                                                    which(JCC_Seurat_Obj$time2 == "GMP"))] <- "NO"
  ...
  plot_df <- data.frame(Patient=names(peakcar_ug),
                        ...
                        Tumor_Burden=c(98, 10, 1, 0, 12, 1, 80, 72, 78, 84, 0, 51, 43, 0),
                        stringsAsFactors = FALSE, check.names = FALSE)
  ...
  for(patient in unique(JCC_Seurat_Obj@meta.data$px)) {
    ...
    ### ordering
    plot_df <- data.frame(cbind(target_file[,time_points,drop=FALSE],
                                matrix(0, nrow(target_file), length(setdiff(unique(JCC_Seurat_Obj$time2), time_points)))),
                          stringsAsFactors = FALSE, check.names = FALSE)
    colnames(plot_df) <- c(time_points, setdiff(unique(JCC_Seurat_Obj$time2), time_points))
    plot_df <- plot_df[,unique(JCC_Seurat_Obj$time2)]
  ...
  ### remove GMP
  total_plot_df <- total_plot_df[,-1]
  
  ### add total column
  total_plot_df$Total <- apply(total_plot_df, 1, sum)
  
  ### ordering
  total_plot_df <- total_plot_df[order(-total_plot_df$Total),]
  
  ### add patient column
  total_plot_df$Px <- sapply(rownames(total_plot_df), function(x) strsplit(x, split = "_", fixed = TRUE)[[1]][1])
  
  ### only with the PI clone size > 10
  target_plot_df <- total_plot_df[which(total_plot_df$Total > 10),]
  
  ### View
  View(target_plot_df)
  
  ### see which clusters they fall in (with top 20 big clone sizes)
  top_20_big_clones <- rownames(target_plot_df)[1:20]
  
  ### get the big clone cluster list
  big_clone_cluster_list <- unlist(sapply(top_20_big_clones, function(x) {
    return(as.character(JCC_Seurat_Obj$AllSeuratClusters[intersect(which(JCC_Seurat_Obj$clonotype_id_by_patient_one_alpha_beta == x),
                                                                   which(JCC_Seurat_Obj$GMP == "PI"))]))
  }))
  
  ### get frequencies of those
  big_clone_clusters <- rep(0, length(unique(big_clone_cluster_list)))
  names(big_clone_clusters) <- unique(big_clone_cluster_list)
  big_clone_clusters <- big_clone_clusters[order(as.numeric(names(big_clone_clusters)))]
  for(clone in names(big_clone_clusters)) {
    big_clone_clusters[clone] <- length(which(big_clone_cluster_list == clone))
  }
  
  ### see the result
  print(big_clone_clusters)
  print(paste(round(big_clone_clusters*100/sum(big_clone_clusters), 2), "%"))
  
  #
  ### make a supplementary table of clonal focusing of each patient
  #
  
  clonal_focusing_table <- data.frame(Px=unique(JCC_Seurat_Obj$px),
                                      Clonal_Focusing_Num=0,
                                      L_One_ES_Ten=0,
                                      One=0,
                                      stringsAsFactors = FALSE, check.names = FALSE)
  
  for(i in 1:nrow(clonal_focusing_table)) {
    clonal_focusing_table$Clonal_Focusing_Num[i] <- length(intersect(which(total_plot_df$Px == clonal_focusing_table$Px[i]),
                                                                     which(total_plot_df$Total > 10)))
    clonal_focusing_table$L_One_ES_Ten[i] <- length(intersect(which(total_plot_df$Px == clonal_focusing_table$Px[i]),
                                                              intersect(which(total_plot_df$Total > 1),
                                                                        which(total_plot_df$Total <= 10))))
    temp_clones <- unique(JCC_Seurat_Obj$clonotype_id_by_patient_one_alpha_beta[intersect(which(JCC_Seurat_Obj$px == clonal_focusing_table$Px[i]),
                                                                                          which(JCC_Seurat_Obj$GMP == "PI"))])
    temp_clones <- temp_clones[which(!is.na(temp_clones))]
    clonal_focusing_table$One[i] <- length(temp_clones) - clonal_focusing_table$Clonal_Focusing_Num[i] - clonal_focusing_table$L_One_ES_Ten[i]
  }
  
  ### save the result
  write.xlsx2(clonal_focusing_table,
              file = paste0(outputDir2, "SJCAR19_PI_Clonal_Focusing.xlsx"),
              sheetName = "Clonal_Focusing_PI",
              row.names = FALSE)
  ...
  ggsave(file = paste0(outputDir2, "Ridgeplot_GMP_Effector_Exp_Suppl_Fig2A.pdf"),
  ...
  ### additional supplementary table
  ### including PI-PI lineages as well
  
  ### make an empty table
  result_df2 <- data.frame(matrix(0, nrow = length(unique(JCC_Seurat_Obj$px))+1, ncol = 4),
                           stringsAsFactors = FALSE, check.names = FALSE)
  rownames(result_df2) <- c(unique(JCC_Seurat_Obj$px), "Total")
  colnames(result_df2) <- c("Alpha Only", "Beta Only", "One Alpha & One Beta", "Exact")
  ...
  for(px in unique(JCC_Seurat_Obj$px)) {
    ...
    ### PI - PI lineages
    temp3 <- NULL
    if(length(unique(JCC_Seurat_Obj$time2[intersect(px_idx,
                                                    which(JCC_Seurat_Obj$GMP == "PI"))])) > 1) {
      for(i in 1:(length(unique(JCC_Seurat_Obj$time2))-1)) {
        lineage_tcrs1 <- unique(JCC_Seurat_Obj$clonotype_id_by_patient_alpha[intersect(px_idx,
                                                                                       intersect(which(JCC_Seurat_Obj$GMP == "PI"),
                                                                                                 which(JCC_Seurat_Obj$time2 == unique(JCC_Seurat_Obj$time2)[i])))])
        lineage_tcrs2 <- unique(JCC_Seurat_Obj$clonotype_id_by_patient_alpha[intersect(px_idx,
                                                                                       intersect(which(JCC_Seurat_Obj$GMP == "PI"),
                                                                                                 which(JCC_Seurat_Obj$time2 == unique(JCC_Seurat_Obj$time2)[i+1])))])
        
        temp2 <- unique(intersect(lineage_tcrs1,
                                  lineage_tcrs2))
    ...
    temp <- unique(c(temp, temp3))
    if(length(temp) > 0) {
      result_df2[px,"Alpha Only"] <- length(temp)
    }
    ...
    if(length(unique(JCC_Seurat_Obj$time2[intersect(px_idx,
                                                    which(JCC_Seurat_Obj$GMP == "PI"))])) > 1) {
      for(i in 1:(length(unique(JCC_Seurat_Obj$time2))-1)) {
        lineage_tcrs1 <- unique(JCC_Seurat_Obj$clonotype_id_by_patient_beta[intersect(px_idx,
                                                                                      intersect(which(JCC_Seurat_Obj$GMP == "PI"),
                                                                                                which(JCC_Seurat_Obj$time2 == unique(JCC_Seurat_Obj$time2)[i])))])
        lineage_tcrs2 <- unique(JCC_Seurat_Obj$clonotype_id_by_patient_beta[intersect(px_idx,
                                                                                      intersect(which(JCC_Seurat_Obj$GMP == "PI"),
                                                                                                which(JCC_Seurat_Obj$time2 == unique(JCC_Seurat_Obj$time2)[i+1])))])
    ...
    temp <- unique(c(temp, temp3))
    if(length(temp) > 0) {
      result_df2[px,"Beta Only"] <- length(temp)
    }
    ...
    if(length(unique(JCC_Seurat_Obj$time2[intersect(px_idx,
                                                    which(JCC_Seurat_Obj$GMP == "PI"))])) > 1) {
      for(i in 1:(length(unique(JCC_Seurat_Obj$time2))-1)) {
        lineage_tcrs1 <- unique(JCC_Seurat_Obj$clonotype_id_by_patient_one_alpha_beta[intersect(intersect(px_idx,
                                                                                                          both_alpha_beta_idx),
                                                                                                intersect(which(JCC_Seurat_Obj$GMP == "PI"),
                                                                                                          which(JCC_Seurat_Obj$time2 == unique(JCC_Seurat_Obj$time2)[i])))])
        lineage_tcrs2 <- unique(JCC_Seurat_Obj$clonotype_id_by_patient_one_alpha_beta[intersect(intersect(px_idx,
                                                                                                          both_alpha_beta_idx),
                                                                                                intersect(which(JCC_Seurat_Obj$GMP == "PI"),
                                                                                                          which(JCC_Seurat_Obj$time2 == unique(JCC_Seurat_Obj$time2)[i+1])))])
    ...
    temp <- unique(c(temp, temp3))
    if(length(temp) > 0) {
      result_df2[px,"One Alpha & One Beta"] <- length(temp)
    }
    ...
    temp <- unique(intersect(gmp_lineage_alpha_tcrs,
                             pi_lineage_alpha_tcrs))
    temp <- temp[which(!is.na(temp))]
    result_df2[px,"Exact"] <- length(temp)
    ...
    if(length(unique(JCC_Seurat_Obj$time2[intersect(px_idx,
                                                    which(JCC_Seurat_Obj$GMP == "PI"))])) > 1) {
      for(i in 1:(length(unique(JCC_Seurat_Obj$time2))-1)) {
        lineage_tcrs1 <- unique(JCC_Seurat_Obj$clonotype_id_by_patient[intersect(px_idx,
                                                                                 intersect(which(JCC_Seurat_Obj$GMP == "PI"),
                                                                                           which(JCC_Seurat_Obj$time2 == unique(JCC_Seurat_Obj$time2)[i])))])
        lineage_tcrs2 <- unique(JCC_Seurat_Obj$clonotype_id_by_patient[intersect(px_idx,
                                                                                 intersect(which(JCC_Seurat_Obj$GMP == "PI"),
                                                                                           which(JCC_Seurat_Obj$time2 == unique(JCC_Seurat_Obj$time2)[i+1])))])
  ...
  ### Total
  result_df2["Total",] <- sapply(1:ncol(result_df2), function(x) {
    return(sum(as.numeric(result_df2[,x])))
  })
  
  ### save the result
  write.xlsx2(data.frame(Patient=rownames(result_df2),
                         result_df2,
                         stringsAsFactors = FALSE, check.names = FALSE),
              file = paste0(outputDir2, "SJCAR19_ALL_Lineage_Different_Definition_Suppl_Table3.xlsx"),
              sheetName = "ALL_Lineage_Info",
              row.names = FALSE)
  ...
  #
  ### Proportional bar plot between seurat clusters and pseudotime states
  #
  ...
  target_clusters <- unique(downsampled_seurat_obj2$AllSeuratClusters)
  target_clusters <- as.character(target_clusters[order(target_clusters)])
  target_states <- unique(downsampled_seurat_obj2$monocle_state)
  target_states <- as.character(target_states[order(target_states)])
  plot_df <- data.frame(Cluster=as.vector(sapply(target_clusters, function(x) rep(x, length(target_states)))),
                        States=rep(target_states, length(target_clusters)),
                        Num=0,
                        Pcnt=0,
                        stringsAsFactors = FALSE, check.names = FALSE)
  ...
  for(i in 1:nrow(plot_df)) {
    plot_df$Num[i] <- length(intersect(which(downsampled_seurat_obj2$AllSeuratClusters == plot_df$Cluster[i]),
                                       which(downsampled_seurat_obj2$monocle_state == plot_df$States[i])))
  }
  ...
  ### add states2 column
  plot_df$States2 <- ""
  plot_df$States2[which(as.numeric(plot_df$Num) != 0)] <- as.character(plot_df$States[which(as.numeric(plot_df$Num) != 0)])
  plot_df$States2[which(plot_df$Pcnt == "")] <- ""
  ...
  plot_df$States <- factor(plot_df$States, levels = target_states)
  
  ### color scale
  sjcar19_colors <- rev(c("#640B11", "#AA4C26", "#D39F3A", "#C09969", "#287B66", "#487A8F", "#3B3B53"))
  names(sjcar19_colors) <- target_states
  show_col(sjcar19_colors)
  
  ### draw a proportional bar plot
  p <- ggplot(data=plot_df, aes_string(x="Cluster", y="Pcnt2", fill="States", label="Pcnt")) +
    geom_bar(position = "stack", stat = "identity") +
    # ggtitle("Proportion of Cells") +
    xlab("Clusters") + ylab("Percentage") +
    # geom_text(size = 3, position = position_stack(vjust = 0.5), vjust = 3, color = "blue") +
    geom_text(aes_string(x="Cluster", y="Pcnt2", label = "States2"),
              position = position_stack(vjust = 0.5),
              size = 12, color = "white") +
    coord_flip() +
    scale_fill_manual(values = sjcar19_colors) +
    ...
    theme(plot.title = element_text(hjust = 0.5, color = "black", face = "bold"),
          ...
          legend.key.size = unit(0.7, 'cm'),
          axis.ticks = element_blank())
  ggsave(file = paste0(outputDir2, "Downsampled_Proportional_Bar_Cluster_State.pdf"), plot = p,
         width = 20, height = 10, dpi = 350)
  
  #
  ### Now in the inverse way
  #
  
  ### calculate percentages
  plot_df$Pcnt <- 0
  state_sum <- rep(0, length(target_states))
  names(state_sum) <- target_states
  for(i in 1:length(target_states)) {
    state_sum[i] <- sum(plot_df[which(plot_df$States == target_states[i]),"Num"])
    plot_df$Pcnt[which(plot_df$States == target_states[i])] <- round(plot_df$Num[which(plot_df$States == target_states[i])] * 100 / state_sum[i], 1)
  }
  ...
  ### add cluster2 column
  plot_df$Cluster2 <- ""
  plot_df$Cluster2[which(as.numeric(plot_df$Num) != 0)] <- as.character(plot_df$Cluster[which(as.numeric(plot_df$Num) != 0)])
  plot_df$Cluster2[which(plot_df$Pcnt == "")] <- ""
  ...
  ### color scale
  sjcar19_colors <- c("#D0B78F", "#8C8781", "#C7904F", "#133D31", "#82A5B8", "#3B3B53", "#4C8493", "#C31517",
                      "#D94C21", "#3E89A8", "#AA4C26",  "#CAA638", "#640B11", "#629488", "#BD7897", "#3C2D16", "#25245D",
                      "#E64E46", "#73BCAA", "#7047C1", "#286278")
  names(sjcar19_colors) <- target_clusters
  show_col(sjcar19_colors)
  
  ### draw a proportional bar plot
  p <- ggplot(data=plot_df, aes_string(x="States", y="Pcnt2", fill="Cluster", label="Pcnt")) +
    geom_bar(position = "stack", stat = "identity") +
    # ggtitle("Proportion of Cells") +
    xlab("States") + ylab("Percentage") +
    # geom_text(size = 3, position = position_stack(vjust = 0.5), vjust = 3, color = "blue") +
    geom_text(aes_string(x="States", y="Pcnt2", label = "Cluster2"),
              position = position_stack(vjust = 0.5),
              size = 12, color = "white") +
    coord_flip() +
    scale_fill_manual(values = sjcar19_colors) +
    ...
    theme(plot.title = element_text(hjust = 0.5, color = "black", face = "bold"),
          ...
          legend.key.size = unit(0.7, 'cm'),
          axis.ticks = element_blank())
  ggsave(file = paste0(outputDir2, "Downsampled_Proportional_Bar_State_Cluster.pdf"), plot = p,
         width = 20, height = 10, dpi = 350)
  ...
  ### color scale
  sjcar19_colors <- c("#C09969", "#286278")
  ...
  ### save the violin plot
  ggsave(file = paste0(outputDir2, "Violin_GEXP_GZMK_Cluster3_8_Suppl_Fig1.pdf"), plot = p, width = 15, height = 10, dpi = 350)
  
  #
  ### Dot plot of all the marker genes with all the clusters
  #
  ...
  p <- DotPlot(JCC_Seurat_Obj,
    ...
    theme(plot.title = element_text(hjust = 0.5, color = "black", face = "bold"),
          ...
          legend.text = element_text(size = 25, color = "black", face = "bold"),
          legend.key.size = unit(0.7, 'cm'))
  ggsave(file = paste0(outputDir2, "Dotplot_CARpos_All_Markers_All_Clusters_FigS1E.pdf"),
  ...
  temp_obj$GMP_Subsisters_End_Up_In_Cluster38_2_CD8[which(temp_obj$GMP_Subsisters_End_Up_In_Cluster38_2_CD8 == "Other_CD8_GMPs")] <- "Other CD8 GMPs"
  ...
  ggsave(paste0(outputDir2, "Ridgeplot_GMP_Precursor_vs_Other_CD8_GMPs.pdf"), plot = p, width = 15, height = 18, dpi = 350)
  
  
  #
  ### Classification for precursor vs other CD8 GMPs
  ### DE genes from training set only and 1k iterations
  ### SVM - Radial only
  #
  
  ### new output directory
  outputDir3 <- paste0(outputDir2, "/Classifier/")
  dir.create(outputDir3)
  ...
  for(i in 1:iteration) {
    ...
    ### draw ROC curve
    pdf(paste0(outputDir3, "Classifier_GMP_Precursor_vs_Other_CD8_GMPs_(", i, ").pdf"),
        width = 5, height = 4)
  ...
  #
  ### Since iteration 1000 is a computationally heavy, I decided to run it on the HPC
  ### make R files and SH files to do the job
  #
  
  ### load the example R script
  r_text <- as.vector(read.csv(file = "./codes/102821_SJCAR19_Classifier_HPC_R_Script.R",
                               sep = "\n", quote = "", header = FALSE)[,1])
  
  ### iteratively generate the R script files
  outputDir4 <- "./codes/Classifier_HPC/"
  dos2unix_path <- "Z:/ResearchHome/SharedResources/Immunoinformatics/hkim8/Tools/dos2unix.exe"
  batch_total <- 100
  for(i in 1:batch_total) {
    r_text[5] <- paste0("batch_num <- ", i)
    
    ### write
    write.table(r_text, file = paste0(outputDir4, "102821_SJCAR19_Classifier_HPC_R_Script", i, ".R"),
                sep="\n", quote=FALSE, row.names=FALSE, col.names=FALSE)
    ### dos2unix to the result
    system(paste(dos2unix_path, paste0(outputDir4, "102821_SJCAR19_Classifier_HPC_R_Script", i, ".R")))
  }
  
  ### load the example SH script
  sh_text <- as.vector(read.csv(file = "./codes/classifier_run.sh",
                                sep = "\n", quote = "", header = FALSE)[,1])
  
  ### generate sh files for submitting to HPC
  for(i in 1:batch_total) {
    sh_text[2] <- paste0("R_FILE=/research/sharedresources/immunoinformatics/hkim8/SJCAR19_classifier/102821_SJCAR19_Classifier_HPC_R_Script", i, ".R")
    
    ### write
    write.table(sh_text, file = paste0(outputDir4, "classifier_run", i, ".sh"),
                sep="\n", quote=FALSE, row.names=FALSE, col.names=FALSE)
    ### dos2unix to the result
    system(paste(dos2unix_path, paste0(outputDir4, "classifier_run", i, ".sh")))
  }
  
  ### generate master sh file
  nextLine <- "\n"
  blank <- " "
  script <- paste0("#!/bin/sh", nextLine)
  
  for(i in 1:batch_total) {
    script <- paste0(script, "bsub -R \"rusage[mem=30000]\" -P SJCAR19 -q standard -J RUN", i, " \"/research/sharedresources/immunoinformatics/hkim8/SJCAR19_classifier/classifier_run", i, ".sh\"", nextLine)
  }
  
  write.table(script, file = paste0(outputDir4, "classifier_master.sh"),
              sep="\n", quote=FALSE, row.names=FALSE, col.names=FALSE)
  system(paste(dos2unix_path, paste0(outputDir4, "classifier_master.sh")))
  
  #
  ### Now the Runs on the clusters were done and it's gathering time
  #
  ...
  ### find out which one was the best among the iterations
  ### I found that max acc is from the iteration 901 and the max auc is from the iteration 541
  ### we will use the max auc one - so, the iteration 541
  max_acc <- max(unlist(acc))
  print(paste(max_acc, "- Iteration", which(unlist(acc) == max_acc)))
  max_auc <- max(unlist(auc))
  print(paste(max_auc, "- Iteration", which(unlist(auc) == max_auc)))
  
  ### create a roc curve with the iteration 541
  ### draw ROC curve
  pdf(paste0(outputDir2, "Classifier_GMP_Precursor_vs_Other_CD8_GMPs_Iteration_541.pdf"),
      width = 5, height = 4)
  plot.roc(roc[[541]], main = paste("Accuracy =", mean(as.numeric(acc[[541]]))),
           legacy.axes = TRUE, print.auc = TRUE, auc.polygon = TRUE,
           xlim = c(1,0), ylim = c(0,1), grid = TRUE, cex.main = 1)
  dev.off()
  
  ### print out the average ACC and AUC from the 1000 iterations
  print(paste("Average ACC = ", round(mean(unlist(acc)), 3)))
  print(paste("Average AUC = ", round(mean(unlist(auc)), 3)))
  
  
  #
  ### build a classifier with strict TCR matching then test it with loosened matching classes
  ### So the training set's class is from one alpha & one beta, and the test set's class is from alpha only or beta only
  #
  ...
  ### GMP clones (alpha chain only)
  gmp_clones_alpha <- unique(JCC_Seurat_Obj$clonotype_id_by_patient_alpha[which(JCC_Seurat_Obj$GMP == "GMP")])
  
  ### PI clones in the cluster3&8 (alpha chain only)
  cluster3_pi_clones_alpha <- unique(JCC_Seurat_Obj$clonotype_id_by_patient_alpha[intersect(which(JCC_Seurat_Obj$GMP == "PI"),
                                                                                            which(JCC_Seurat_Obj$AllSeuratClusters == "3"))])
  cluster8_pi_clones_alpha <- unique(JCC_Seurat_Obj$clonotype_id_by_patient_alpha[intersect(which(JCC_Seurat_Obj$GMP == "PI"),
                                                                                            which(JCC_Seurat_Obj$AllSeuratClusters == "8"))])
  cluster38_pi_clones_alpha <- unique(c(cluster3_pi_clones_alpha, cluster8_pi_clones_alpha))
  
  ### remove NA clones (alpha chain only)
  gmp_clones_alpha <- gmp_clones_alpha[which(!is.na(gmp_clones_alpha))]
  cluster38_pi_clones_alpha <- cluster38_pi_clones_alpha[which(!is.na(cluster38_pi_clones_alpha))]
  
  ### define GMP precursor cells with alpha only
  ### CD4: 247 cells, CD8: 466 cells, TOTAL: 724 cells
  precursor_clones_alpha <- intersect(gmp_clones_alpha, cluster38_pi_clones_alpha)
  JCC_Seurat_Obj$GMP_Prescursors_End_Up_In_Cluster38_Alpha <- NA
  JCC_Seurat_Obj$GMP_Prescursors_End_Up_In_Cluster38_Alpha[intersect(which(JCC_Seurat_Obj$clonotype_id_by_patient_alpha %in% precursor_clones_alpha),
                                                                     which(JCC_Seurat_Obj$GMP == "GMP"))] <- "YES"
  JCC_Seurat_Obj$GMP_Prescursors_End_Up_In_Cluster38_Alpha[intersect(which(JCC_Seurat_Obj$GMP == "GMP"),
                                                                     which(is.na(JCC_Seurat_Obj$GMP_Prescursors_End_Up_In_Cluster38_Alpha)))] <- "NO"
  
  ### define test cells (independent from the training set)
  JCC_Seurat_Obj$Test_Class_Alpha <- NA
  JCC_Seurat_Obj$Test_Class_Alpha[setdiff(which(JCC_Seurat_Obj$GMP_Prescursors_End_Up_In_Cluster38_Alpha == "YES"),
                                          which(JCC_Seurat_Obj$GMP_Subsisters_End_Up_In_Cluster38_2_CD8 == "GMP_Subsisters_End_Up_In_Cluster_3_And_8"))] <- "YES"
  JCC_Seurat_Obj$Test_Class_Alpha[setdiff(which(JCC_Seurat_Obj$GMP_Prescursors_End_Up_In_Cluster38_Alpha == "NO"),
                                          which(JCC_Seurat_Obj$GMP_Subsisters_End_Up_In_Cluster38_2_CD8 == "Other_CD8_GMPs"))] <- "NO"
  
  
  ### GMP clones (beta chain only)
  gmp_clones_beta <- unique(JCC_Seurat_Obj$clonotype_id_by_patient_beta[which(JCC_Seurat_Obj$GMP == "GMP")])
  
  ### PI clones in the cluster3&8 (beta chain only)
  cluster3_pi_clones_beta <- unique(JCC_Seurat_Obj$clonotype_id_by_patient_beta[intersect(which(JCC_Seurat_Obj$GMP == "PI"),
                                                                                          which(JCC_Seurat_Obj$AllSeuratClusters == "3"))])
  cluster8_pi_clones_beta <- unique(JCC_Seurat_Obj$clonotype_id_by_patient_beta[intersect(which(JCC_Seurat_Obj$GMP == "PI"),
                                                                                          which(JCC_Seurat_Obj$AllSeuratClusters == "8"))])
  cluster38_pi_clones_beta <- unique(c(cluster3_pi_clones_beta, cluster8_pi_clones_beta))
  
  ### remove NA clones (beta chain only)
  gmp_clones_beta <- gmp_clones_beta[which(!is.na(gmp_clones_beta))]
  cluster38_pi_clones_beta <- cluster38_pi_clones_beta[which(!is.na(cluster38_pi_clones_beta))]
  
  ### define GMP precursor cells with beta only
  ### CD4: 46 cells, CD8: 413 cells, NA: 34 cells, TOTAL: 493 cells - because of the proportion, the control is CD8+ GMP
  precursor_clones_beta <- intersect(gmp_clones_beta, cluster38_pi_clones_beta)
  JCC_Seurat_Obj$GMP_Prescursors_End_Up_In_Cluster38_Beta <- NA
  JCC_Seurat_Obj$GMP_Prescursors_End_Up_In_Cluster38_Beta[intersect(which(JCC_Seurat_Obj$clonotype_id_by_patient_beta %in% precursor_clones_beta),
                                                                    which(JCC_Seurat_Obj$GMP == "GMP"))] <- "YES"
  JCC_Seurat_Obj$GMP_Prescursors_End_Up_In_Cluster38_Beta[intersect(intersect(which(JCC_Seurat_Obj$GMP == "GMP"),
                                                                              which(JCC_Seurat_Obj$CD4_CD8_by_Clusters == "CD8")),
                                                                    which(is.na(JCC_Seurat_Obj$GMP_Prescursors_End_Up_In_Cluster38_Beta)))] <- "NO"
  
  ### define test cells (independent from the training set)
  JCC_Seurat_Obj$Test_Class_Beta <- NA
  JCC_Seurat_Obj$Test_Class_Beta[setdiff(which(JCC_Seurat_Obj$GMP_Prescursors_End_Up_In_Cluster38_Beta == "YES"),
                                         which(JCC_Seurat_Obj$GMP_Subsisters_End_Up_In_Cluster38_2_CD8 == "GMP_Subsisters_End_Up_In_Cluster_3_And_8"))] <- "YES"
  JCC_Seurat_Obj$Test_Class_Beta[setdiff(which(JCC_Seurat_Obj$GMP_Prescursors_End_Up_In_Cluster38_Beta == "NO"),
                                         which(JCC_Seurat_Obj$GMP_Subsisters_End_Up_In_Cluster38_2_CD8 == "Other_CD8_GMPs"))] <- "NO"
  
  
  ### build a classifier with one alpha/one beta
  ...
  JCC_Seurat_Obj_Alpha <- subset(JCC_Seurat_Obj,
                                 cells = rownames(JCC_Seurat_Obj@meta.data)[])
  ...
  ### build the classifier 10 times with random down-sampling
  acc_alpha <- vector("list", length = iteration)
  roc_alpha <- vector("list", length = iteration)
  auc_alpha <- vector("list", length = iteration)
  acc_beta <- vector("list", length = iteration)
  roc_beta <- vector("list", length = iteration)
  auc_beta <- vector("list", length = iteration)
  for(i in 1:iteration) {
    
    writeLines(paste(i, "/", iteration))
    
    ### get downsampling indicies
    training_downsampled_control_idx <- sample(which(JCC_Seurat_Obj$GMP_Subsisters_End_Up_In_Cluster38_2_CD8 == "Other_CD8_GMPs"), sampleNum)
    downsample_idx <- c(sample(which(JCC_Seurat_Obj$GMP_Subsisters_End_Up_In_Cluster38_2_CD8 == "GMP_Subsisters_End_Up_In_Cluster_3_And_8"), sampleNum),
                        training_downsampled_control_idx)
    downsample_idx2 <- c(sample(which(JCC_Seurat_Obj$Test_Class_Alpha == "YES"), sampleNum),
                         sample(setdiff(which(JCC_Seurat_Obj$GMP_Prescursors_End_Up_In_Cluster38_Alpha == "NO"),
                                        training_downsampled_control_idx), sampleNum))
    downsample_idx3 <- c(sample(which(JCC_Seurat_Obj$Test_Class_Beta == "YES"), sampleNum),
                         sample(setdiff(which(JCC_Seurat_Obj$GMP_Prescursors_End_Up_In_Cluster38_Beta == "NO"),
                                        training_downsampled_control_idx), sampleNum))
    
    ### get normalized data
    input_data <- normalizeRNASEQwithVST(readCount = data.frame(JCC_Seurat_Obj@assays$RNA@counts[,downsample_idx],
                                                                stringsAsFactors = FALSE, check.names = FALSE))
    test_data1 <- normalizeRNASEQwithVST(readCount = data.frame(JCC_Seurat_Obj@assays$RNA@counts[,downsample_idx2],
                                                                stringsAsFactors = FALSE, check.names = FALSE))
    test_data2 <- normalizeRNASEQwithVST(readCount = data.frame(JCC_Seurat_Obj@assays$RNA@counts[,downsample_idx3],
                                                                stringsAsFactors = FALSE, check.names = FALSE))
    
    ### only use the genes that are worth
    classifier_genes1 <- intersect(de_genes, intersect(rownames(input_data),
                                                       rownames(test_data1)))
    classifier_genes2 <- intersect(de_genes, intersect(rownames(input_data),
                                                       rownames(test_data2)))
    
    ### row - col change
    input_data1 <- data.frame(t(input_data[classifier_genes1,,drop=FALSE]), stringsAsFactors = FALSE, check.names = FALSE)
    test_data1 <- data.frame(t(test_data1[classifier_genes1,,drop=FALSE]), stringsAsFactors = FALSE, check.names = FALSE)
    input_data2 <- data.frame(t(input_data[classifier_genes2,,drop=FALSE]), stringsAsFactors = FALSE, check.names = FALSE)
    test_data2 <- data.frame(t(test_data2[classifier_genes2,,drop=FALSE]), stringsAsFactors = FALSE, check.names = FALSE)
    
    ### annotate class for the data
    input_data1$Class <- factor(JCC_Seurat_Obj$Classification_Class[downsample_idx],
                                levels = c("YES", "NO"))
    test_data1$Class <- factor(c(rep("YES", sampleNum),
                                 rep("NO", sampleNum)),
                               levels = c("YES", "NO"))
    input_data2$Class <- factor(JCC_Seurat_Obj$Classification_Class[downsample_idx],
                                levels = c("YES", "NO"))
    test_data2$Class <- factor(c(rep("YES", sampleNum),
                                 rep("NO", sampleNum)),
                               levels = c("YES", "NO"))
    
    ### get accuracy and pred prob (not AUC yet)
    model <- train(Class~., data=input_data1, method=methodTypes, trControl = train_control)
    pred_result <- predict(model, newdata = test_data1)
    acc_alpha[[i]] <- round((sum(pred_result == test_data1$Class) / nrow(test_data1)), 3)
    pred_result <- predict(model, newdata = test_data1, type = "prob")
    roc_alpha[[i]] <- pred_result$YES
    auc_alpha[[i]] <- roc(test_data1$Class, roc_alpha[[i]])
    
    model <- train(Class~., data=input_data2, method=methodTypes, trControl = train_control)
    pred_result <- predict(model, newdata = test_data2)
    acc_beta[[i]] <- round((sum(pred_result == test_data2$Class) / nrow(test_data2)), 3)
    pred_result <- predict(model, newdata = test_data2, type = "prob")
    roc_beta[[i]] <- pred_result$YES
    auc_beta[[i]] <- roc(test_data2$Class, roc_beta[[i]])
    
  }
  
  ### print out the average ACC and AUC
  print(paste0("Classifier - Training with One alpha & beta and Testing with alpha only - ACC: ", mean(unlist(acc_alpha))))
  print(paste0("Classifier - Training with One alpha & beta and Testing with alpha only - AUC: ", mean(sapply(auc_alpha, function(x) x$auc))))
  
  print(paste0("Classifier - Training with One alpha & beta and Testing with beta only - ACC: ", mean(unlist(acc_beta))))
  print(paste0("Classifier - Training with One alpha & beta and Testing with beta only - AUC: ", mean(sapply(auc_beta, function(x) x$auc))))
  
  
  #
  ### Compare the clonal diversity between Turtle data and ours
  #
  
  ### load turtle data first
  f <- list.files(path = "./data/GSE125881_RAW/")
  ...
  for(i in 1:length(f)) {
    temp <- read.csv(file = paste0("./data/GSE125881_RAW/", f[i]),
                     stringsAsFactors = FALSE, check.names = FALSE)
  ...
  ### load our tcr data
  f2 <- list.files(path = "./data/ContigAnnots/")
  ...
  for(i in 1:length(f2)) {
    our_tcrs[[i]] <- read.csv(file = paste0("./data/ContigAnnots/", f2[i]),
                              stringsAsFactors = FALSE, check.names = FALSE)
  }
  ...
  plot_df <- data.frame(Name=c(names(normalized_unique_tcr_num_turtle_beta),
                        ...
                        Source=c(rep("Turtle", length(normalized_unique_tcr_num_turtle_beta)),
                                 rep("Ours", length(normalized_unique_tcr_num_ours_beta))),
  ...
  ### bar plot - shannon idx
  p <- ggplot(data=plot_df2, aes_string(x="Name", y="ShannonIdx", fill="Source")) +
    geom_bar(stat = "identity") +
    ggtitle("Diversity - Shannon Index") +
    labs(fill = "") +
    scale_fill_manual(values = c("#487A8F", "#640B11")) +
    # geom_text(size = 3, position = position_stack(vjust = 1)) +
    ...
    theme(plot.title = element_text(hjust = 0.5, vjust = 0.5, size = 30, color = "black", face = "bold"),
          ...
          axis.text = element_text(angle = 0, size = 25, vjust = 0.5, hjust = 0.5, color = "black", face = "bold"),
          axis.ticks = element_blank())
  ggsave(file = paste0(outputDir2, "Barplot_Clonal_Diversity_Shannon_Index_Beta_GMP.pdf"),
         plot = p, width = 25, height = 15, dpi = 350)
  ...
  p <- ggplot(plot_df2, aes_string(x="Source", y="ShannonIdx")) +
    ...
    xlab("") + ylab("") +
    labs(col="") +
    scale_color_manual(values = c("#487A8F", "#640B11")) +
    theme_classic(base_size = 30) +
    theme(plot.title = element_text(hjust = 0.5, vjust = 0.5, size = 30, color = "black", face = "bold"),
          plot.subtitle = element_text(hjust = 0, vjust = 0.5, size = 25, color = "black", face = "bold"),
          legend.text = element_text(hjust = 0, vjust = 0.5, size = 30, color = "black", face = "bold"),
          ...
          axis.text = element_text(angle = 0, size = 25, vjust = 0.5, hjust = 0.5, color = "black", face = "bold"),
          axis.ticks = element_blank())
  ggsave(file = paste0(outputDir2, "Beeswarm_Clonal_Diversity_Shannon_Index_Beta_GMP.pdf"),
         plot = p, width = 12, height = 8, dpi = 350)
  
  ### only Wk2/Wk3/Wk4
  plot_df3 <- plot_df[which(plot_df$Time %in% c("Wk2", "Wk3", "Wk4")),]
  
  ### bar plot - shannon idx
  p <- ggplot(data=plot_df3, aes_string(x="Name", y="ShannonIdx", fill="Source")) +
    geom_bar(stat = "identity") +
    ggtitle("Diversity - Shannon Index") +
    labs(fill = "") +
    scale_fill_manual(values = c("#487A8F", "#640B11")) +
    ...
    theme(plot.title = element_text(hjust = 0.5, vjust = 0.5, size = 30, color = "black", face = "bold"),
          ...
          axis.text = element_text(angle = 0, size = 25, vjust = 0.5, hjust = 0.5, color = "black", face = "bold"),
          axis.ticks = element_blank())
  ggsave(file = paste0(outputDir2, "Barplot_Clonal_Diversity_Shannon_Index_Beta_Wk3.pdf"),
         plot = p, width = 25, height = 15, dpi = 350)
  
  ### beeswarm plot
  p <- ggplot(plot_df3, aes_string(x="Source", y="ShannonIdx")) +
    geom_boxplot() +
    ggtitle("Diversity - Shannon Index") +
    geom_beeswarm(aes_string(col="Source"), na.rm = TRUE, show.legend = FALSE, size = 5, cex = 3) +
    stat_compare_means(size = 5, hjust = -0.5) +
    ...
    theme(plot.title = element_text(hjust = 0.5, vjust = 0.5, size = 30, color = "black", face = "bold"),
          ...
          axis.text = element_text(angle = 0, size = 25, vjust = 0.5, hjust = 0.5, color = "black", face = "bold"),
          axis.ticks = element_blank())
  ggsave(file = paste0(outputDir2, "Beeswarm_Clonal_Diversity_Shannon_Index_Beta_Wk3.pdf"),
         plot = p, width = 12, height = 8, dpi = 350)
  
  ### just check # lineages with beta (compare turtle & ours)
  writeLines(paste0("# Lineages in CLL-1_IP: ",
                    length(unique(intersect(names(turtle_tcrs_beta[["CLL-1_IP"]]), c(names(turtle_tcrs_beta[["CLL-1_d21"]]),
                                                                                     names(turtle_tcrs_beta[["CLL-1_d38"]]),
                                                                                     names(turtle_tcrs_beta[["CLL-1_d112"]])))))))
  writeLines(paste0("# Lineages in CLL-2_IP: ",
                    length(unique(intersect(names(turtle_tcrs_beta[["CLL-2_IP"]]), c(names(turtle_tcrs_beta[["CLL-2_d12"]]),
                                                                                     names(turtle_tcrs_beta[["CLL-2_d29"]]),
                                                                                     names(turtle_tcrs_beta[["CLL-2_d83"]])))))))
  writeLines(paste0("# Lineages in NHL-6_IP: ",
                    length(unique(intersect(names(turtle_tcrs_beta[["NHL-6_IP"]]), c(names(turtle_tcrs_beta[["NHL-6_d12"]]),
                                                                                     names(turtle_tcrs_beta[["NHL-6_d29"]]),
                                                                                     names(turtle_tcrs_beta[["NHL-6_d102"]])))))))
  writeLines(paste0("# Lineages in NHL-7_IP: ",
                    length(unique(intersect(names(turtle_tcrs_beta[["NHL-7_IP"]]), c(names(turtle_tcrs_beta[["NHL-7_d12"]]),
                                                                                     names(turtle_tcrs_beta[["NHL-7_d28"]]),
                                                                                     names(turtle_tcrs_beta[["NHL-7_d89"]])))))))
  
  
  
  
  #
  ### KS test - suppl figure
  #
  ...
  max_clone_size <- Reduce(max, c(subsister_clone_sizes_gmp, rest_carpos_clone_sizes_gmp,
                                  subsister_clone_sizes_pi, rest_carpos_clone_sizes_pi))
  png(filename = paste0(outputDir2, "/", "Cumulative_Clone_Sizes_FigS4B.png"), width = 2000, height = 1500, res = 350)
  plot(ecdf(subsister_clone_sizes_gmp), col="#640B11", lwd = 1,
       main = "Cumulative Clone Sizes",
       sub = paste0("KS test p-value: ",
                    "GMP=", formatC(ks.test(subsister_clone_sizes_gmp, rest_carpos_clone_sizes_gmp)$p.value, format = "e", digits = 2), ", ",
                    "PI=", formatC(ks.test(subsister_clone_sizes_pi, rest_carpos_clone_sizes_pi)$p.value, format = "e", digits = 2)),
  ...
  lines(ecdf(rest_carpos_clone_sizes_gmp), col="#487A8F", lwd = 1)
  lines(ecdf(subsister_clone_sizes_pi), col="#D39F3A", lwd = 1)
  lines(ecdf(rest_carpos_clone_sizes_pi), col="#3B3B53", lwd = 1)
  legend("bottomright", 
         legend=c("GMP CAR+ Lineages", "The Rest GMP CAR+",
                  "PI CAR+ Lineages","The Rest PI CAR+"),
         col=c("#640B11", "#487A8F","#D39F3A","#3B3B53"),
         pch=15)
  dev.off()
  
  pdf(file = paste0(outputDir2, "/", "Cumulative_Clone_Sizes_FigS4B.pdf"), width = 5, height = 4)
  plot(ecdf(subsister_clone_sizes_gmp), col="#640B11", lwd = 1,
       main = "Cumulative Clone Sizes",
       sub = paste0("KS test p-value: ",
                    "GMP=", formatC(ks.test(subsister_clone_sizes_gmp, rest_carpos_clone_sizes_gmp)$p.value, format = "e", digits = 2), ", ",
  ...
  ### look at how many precursor GMP / Other CD8 GMP in each patient
  unique_px <- unique(JCC_Seurat_Obj$px)
  for(px in unique_px) {
    writeLines(paste(px, length(intersect(which(JCC_Seurat_Obj$px == px),
                                          which(JCC_Seurat_Obj$GMP_Subsisters_End_Up_In_Cluster38_2_CD8 == "GMP_Subsisters_End_Up_In_Cluster_3_And_8"))), ",",
                     length(intersect(which(JCC_Seurat_Obj$px == px),
  ...
  ### There are some patients that have no/few precursor cells, so we will skip those in one-patient training
  ### Only 06, 08, 10, 11 will be used in training
  training_px <- c("SJCAR19-06", "SJCAR19-08", "SJCAR19-10", "SJCAR19-11")
  
  acc <- vector("list", length = length(training_px))
  names(acc) <- training_px
  roc <- vector("list", length = length(training_px))
  names(roc) <- training_px
  for(px in training_px) {
    ...
    ### get normalized data
    input_data <- normalizeRNASEQwithVST(readCount = data.frame(JCC_Seurat_Obj@assays$RNA@counts[,training_idx],
                                                                stringsAsFactors = FALSE, check.names = FALSE),
                                         filter_thresh = 0)
    test_data <- normalizeRNASEQwithVST(readCount = data.frame(JCC_Seurat_Obj@assays$RNA@counts[,test_idx],
                                                               stringsAsFactors = FALSE, check.names = FALSE),
                                        filter_thresh = 0)
    
    ### annotate class for the input data
    input_data <- data.frame(t(input_data[de_genes,,drop=FALSE]), stringsAsFactors = FALSE, check.names = FALSE)
    input_data$Class <- factor(JCC_Seurat_Obj$Classification_Class[training_idx],
                               levels = c("YES", "NO"))
    
    ### annotate class for the test data
    test_data <- data.frame(t(test_data[de_genes,,drop=FALSE]), stringsAsFactors = FALSE, check.names = FALSE)
    test_data$Class <- factor(JCC_Seurat_Obj$Classification_Class[test_idx],
                              levels = c("YES", "NO"))
    
    ### get accuracy and pred prob (not AUC yet)
    model <- train(Class~., data=input_data, method=methodTypes, trControl = train_control)
    pred_result <- predict(model, newdata = test_data)
    acc[[px]] <- round((sum(pred_result == test_data$Class) / nrow(test_data)), 3)
    pred_result <- predict(model, newdata = test_data, type = "prob")
    roc[[px]] <- roc(test_data$Class, pred_result$YES)
